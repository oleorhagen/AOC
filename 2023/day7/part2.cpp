...
int nr_of_jokers(hand h) {
	return std::count_if(h.cbegin(), h.cend(), [](char c) { return c == 'J'; });
}
...
int hand_value(hand h) {
	...
	for (const auto &[key, val] : m) {
		hand_val = std::max(hand_val, val + nr_of_jokers(h));
	}
	if (m.size() == 0) {
		hand_val = 5;
	}
	...
	std::cerr << "Got value: " << std::to_string(hand_val + nr_of_jokers(h))
			  << " hand_val: " << hand_val << std::endl;
	std::cerr << "Hand: " << h << std::endl;
	assert(false);
}
