...
vector<pos> antinodes(pos _a, pos _b) {
  ...
  auto first_antinode = a + (a - b);
  auto second_antinode = b - (a - b);
...
int main(int argc, char* argv[]) {
  ...
  /* Test the antinode function */
  pos point_a{3, 4};
  pos point_b{5, 6};

  const auto calculated_antinodes = antinodes(point_a, point_b);

  std::cout << "Calculated antinodes for {3,4}, {5,6}: " << calculated_antinodes
            << "\n";

  std::vector<pos> viz{point_a, point_b};
  viz.insert(viz.end(), calculated_antinodes.begin(),
             calculated_antinodes.end());

  std::cout << "Viz: " << viz << "\n";

  visualiseGrid(m.size(), viz);
